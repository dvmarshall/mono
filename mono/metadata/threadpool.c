...
mono_thread_pool_init (void)
{
	...
	threadpool_init (&async_tp, MAX (thread_count, 8), MAX (100 * cpu_count, thread_count), async_invoke_thread);
	threadpool_init (&async_io_tp, MAX(cpu_count * 2, 8), MAX (cpu_count * 4, 16), async_invoke_thread);
